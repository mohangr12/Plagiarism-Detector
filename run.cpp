...
char const * SourceFile = "E:\\Plagiarism Detector\\SourceFile";

// Import text files which we want to check from the Target folder.
char const * target_folder = "E:\\Plagiarism Detector\\Target";
    ...
    if ((dir = opendir (target_folder)) != NULL) {
        while ((dir_object = readdir (dir)) != NULL)
            if(endswith(string(dir_object->d_name), ".txt")){
                ...
                cout<<"Test 1 score: "<<fixed<<setprecision(score_accuracy)<<test[0]<<"/10"<<endl <<endl;
                cout<<"Test 2 score: "<<fixed<<setprecision(score_accuracy)<<test[1]<<"/10"<<endl <<endl;
                cout<<"Test 3 score: "<<fixed<<setprecision(score_accuracy)<<test[2]<<"/10"<<endl <<endl;
